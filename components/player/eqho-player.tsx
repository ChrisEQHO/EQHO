...
export interface EqhoPlayerProps {
  /**
   * When true, the player runs as the public interactive demo:
   * no Supabase auth / login redirect, playlists are seeded from the read-only
   * /api/demo snapshot, and every cloud/account/upload/offline path is disabled
...
export function EqhoPlayer({ demoMode = false, presentation = "standalone" }: EqhoPlayerProps) {
  ...
  // Sidebar navigation items (desktop only). In demo mode the account-only
  // "EQHO Cloud" area is hidden (it requires login and touches customer data).
  const sidebarItems = (
    [
  ...
  ).filter((item) => !(demoMode && item.page === "cloud"));
  ...
  useEffect(() => {
    // Demo mode (public /features): run the player fully logged-out. We never
    // call Supabase, never redirect to /login, and deliberately leave `user`
    // null so EVERY cloud/account path (all gated on `user`) stays inert and
    // no customer data is ever touched. Access is already "granted" above.
    if (demoMode) {
      setAuthChecked(true);
      return;
    }
  ...
  useEffect(() => {
    // Demo mode is in-memory only: never read the visitor's IndexedDB. Mark the
    // load complete so the rest of the player behaves normally with an empty
    // starting session (the demo playlists are seeded separately below).
    if (demoMode) {
  ...
  useEffect(() => {
    if (demoMode) return; // Demo mode is in-memory only: never write IndexedDB.
  ...
  useEffect(() => {
    if (demoMode) return; // Demo mode is in-memory only: never write localStorage.
  ...
  useEffect(() => {
    if (demoMode) return; // Demo playlists are ephemeral: never write IndexedDB.
  ...
  // Demo mode: seed the library from the public, read-only /api/demo snapshot
  // instead of the visitor's IndexedDB. Each track streams by opaque id from the
  // demo audio endpoint (no File, no blob, no customer storage). This is the only
  // source of playlists in demo mode.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await res.json();
        if (cancelled) return;
        const playlistsIn: Array<{ id: string; name: string; tracks: Array<{ id: string; name: string; durationSeconds: number }> }> =
          data?.enabled && Array.isArray(data.playlists) ? data.playlists : [];
        if (playlistsIn.length === 0) {
          setDemoLoadState("empty");
          return;
        }
        const seeded = playlistsIn.map((p) => ({
          id: p.id,
          name: p.name,
          tracks: p.tracks.map((t) => ({
            ...
            fileName: `${t.name}.mp3`,
            // Streams from the public demo endpoint; resolved to an R2 demo/ key
            // server-side. Same-origin URL plays directly in the <audio> element.
            url: `/api/demo?action=audio&track=${encodeURIComponent(t.id)}`,
            durationSeconds: t.durationSeconds,
            uploadedAt: new Date().toISOString(),
          })),
        }));
  ...
  useEffect(() => {
    if (demoMode) return; // No device-download metadata in demo mode.
    try {
  ...
  return (
    ...
    >
      ...
      {/* Branded promo banner — DESKTOP ONLY. Removed on mobile and iPad (both
          ...
          Hidden entirely in the public demo (it advertises the subscription). */}
      {!demoMode && isDesktopDevice && (
          ...
          <div className="col-span-3 col-start-2 h-full overflow-y-auto pb-6 rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                {/* Account-only settings (cloud sync, account, danger zone) are
                    hidden in the public demo — they require login and touch
                    customer data. */}
                {!demoMode && (
                <>
              ...
              {mobileTab === "settings" && (
                <div className="h-full flex flex-col">
                  <Card className="flex-1 bg-white/[0.03] border-white/10 backdrop-blur-sm p-3 flex flex-col overflow-hidden">
                    ...
                    {/* Account-only settings (cloud sync + subscription) are
                        hidden in the public demo. */}
                    {!demoMode && (
                    <>
                    {/* EQHO Cloud - Sync & Backup */}
                    ...
                    {/* Account / Logout — hidden in the public demo (login only). */}
                    {!demoMode && (
