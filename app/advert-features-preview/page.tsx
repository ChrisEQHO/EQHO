...
// Download-only preview surface for the EQHO Player FEATURES advert.
...
import { EqhoAdvertFeatures } from "@/components/advert/eqho-advert-features"

const ADVERT_W = 1080
const ADVERT_H = 1350

export default function AdvertFeaturesPreviewPage() {
  ...
  const handleDownload = useCallback(async () => {
    const node = document.getElementById("eqho-advert-features")
    if (!node) return
    setDownloading(true)
    try {
      ...
      link.download = "eqho-player-features-advert-1080x1350.jpg"
      link.href = dataUrl
      link.click()
    } finally {
